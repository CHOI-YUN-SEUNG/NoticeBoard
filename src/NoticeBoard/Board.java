...
public class Board {
	...
	private void login() {
		if (checkNull("이미 로그인 되어 있습니다")) {
			...
			if (user != null && user.getPassword().equals(password)) {
				nowUser = id;
				if (user.getId().equals(userManager.getAdmin().getId()))
					isAdmin = true;
	...
	private void viewUserPosts() {
		...
		Collections.sort(posts, Comparator.comparingInt(UserPost::getUpvotes).reversed());
		for (int i = 0; i < posts.size(); i++) {
